...
function Admin() {
  ...
  const handleCancelWithMessage = async () => {
    ...
    try {
      const response = await fetch(
        `${BOOKING_API_URL}/${selectedBooking._id}?cancelMessage=${encodeURIComponent(cancelMessage)}`,
        {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' }
        }
  ...
  const handleDeleteBooking = async (id) => {
    ...
    setLoading(true);
    try {
      await fetch(`${BOOKING_API_URL}/${id}`, { method: 'DELETE' });
  ...
  // ---------- 🟢 Assign Crew Functions ----------
  const openAssignPopup = async (booking) => {
    ...
    try {
      ...
      // Get booked crew for this time slot
      const bookedResponse = await fetch(`${BOOKING_API_URL}?date=${booking.eventDate}&time=${booking.eventTime}`);
      const bookedData = await bookedResponse.json();
      
      const bookedCrewIds = bookedData.flatMap(b => b.assignedCrew || []);
      let available = allCrew.filter(c => !bookedCrewIds.includes(c._id));
      // filter by service/unit matching the booking's requested services
      const services = Array.isArray(booking.serviceType) ? booking.serviceType : [booking.serviceType];
      const allowedUnits = services.flatMap(s => {
        if (s === 'Photographer') return ['Photography'];
        if (s === 'Videographer') return ['Videography'];
        if (s === 'Broadcaster') return ['Broadcasting'];
        if (s === 'All') return ['Photography', 'Videography', 'Broadcasting'];
        return [];
      });

      available = available.filter(c => allowedUnits.includes(c.unit));
  ...
  return (
    <div className={styles.adminContainer}>
      ...
      {/* ===== ASSIGN CREW POPUP ===== */}
      {showAssignPopup && selectedAssignBooking && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h2>👥 Assign Crew Members</h2>
              <button className={styles.modalClose} onClick={closeAssignPopup}>✕</button>
            </div>
            <div className={styles.modalBody}>
              ...
              <div className={styles.assignCrewSection}>
                ...
                {availableCrew.length === 0 ? (
                  <div className={styles.noCrewAvailable}>
                    <p>⚠️ No crew members available at this time.</p>
                    <p className={styles.noCrewNote}>
                      All crew members are already assigned to other events at this time.
                    </p>
                  </div>
                ) : (
            ...
            <div className={styles.modalFooter}>
              <button 
                ...
                disabled={assignLoading || availableCrew.length === 0}
              >
